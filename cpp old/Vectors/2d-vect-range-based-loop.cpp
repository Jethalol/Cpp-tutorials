...
/* we can't print vect1D itself because it is nothing but a vector of vector, so first we loop through entire vector
and then copy its element. Just like in 2-D array, arr[0] represent the entire first row and arr[0][0] and so on represent
element of respective row. You will better understand the concept in next example when we'll loop using traditional
for loop */